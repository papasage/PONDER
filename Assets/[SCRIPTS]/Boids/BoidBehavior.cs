...
public class BoidBehavior : MonoBehaviour
{
    ...
    void SetSize(float size)
    {
        ...
        // Calculate offset based on the size of the fish's mesh
        float bubbleOffset = size * 1f; // Adjust this multiplier based on your needs

        // Apply offset to the position of the speech bubble
        chatBubble.transform.localPosition = new Vector3(0f, bubbleOffset, 0.7f);
    }
    void GetAllBoids()
    {
    ...
    void ApplyHookedBehavior()
    {
        ...
        {
            //move away from rod
            Vector3 awayDirection = transform.position - fishingRod.transform.position;
            ...
            //transform.Translate(-(fishingRod.transform.position).normalized * .05f * Time.deltaTime);
            ...
            Debug.DrawLine(transform.position, fishingRod.transform.position, Color.magenta);
    ...
    void Eat(BoidBehavior boid)
    {
        ...
        if (boid.isHooked)
        {
            ...
            if (comboMeter > 1)
            {
                AudioManager.instance.Combo(1 + (comboMeter * .1f));
            }
